...
const DriversList = () => {
    ...
    const { filters } = useSelector((state) => state.drivers.data);
    const { currentPage, driverStatus, query } = filters;
    ...
    useEffect(() => {
        isSuccess && dispatch(getDrivers());
    }, [isSuccess]);

    useEffect(() => {
        dispatch(getDrivers());
    }, [currentPage, driverStatus, query, dispatch]);
